...
export const DEBQ_REFERENCE={restrained:2.4,emotional:1.8,external:2.7} as const;
type DebqScale={label:string;average:number;min:number;max:number;items:readonly number[];referenceMean:number;difference:number;elevated:boolean;interpretation:string};
...
export function scoreDebq(answers:Map<number,number>):DebqResult|null{
  ...
  const scale=(key:keyof typeof DEBQ_ITEMS):DebqScale=>{const items=DEBQ_ITEMS[key];const sum=items.reduce((s,item)=>s+(item===31?6-answers.get(item)!:answers.get(item)!),0);const average=round2(sum/items.length),referenceMean=DEBQ_REFERENCE[key],elevated=average>referenceMean;return{label:labels[key],average,min:1,max:5,items,referenceMean,difference:round2(average-referenceMean),elevated,interpretation:elevated?'Повышенная выраженность':'Не превышает нормативный ориентир'}};
