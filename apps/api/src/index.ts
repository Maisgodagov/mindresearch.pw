...
app.get('/api/admin/surveys/:id/results',requireAuth,async(req:AuthRequest,res,next)=>{try{
  ...
  const [distribution]=await db.query<any[]>(`SELECT q.code,q.text,a.value,COUNT(*) count FROM answers a JOIN questions q ON q.id=a.question_id JOIN response_sessions rs ON rs.id=a.session_id WHERE rs.survey_id=? GROUP BY q.id,a.value ORDER BY q.position`,[req.params.id]);res.json({sessions,distribution});
}catch(e){next(e)}});
