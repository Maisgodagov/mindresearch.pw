...
type NspsScore={label:string;score:number;minScore:number;maxScore:number;items:number[]};
...
function MethodResult({group}:{group?:AnswerGroup}){
  ...
  if(group.code==='test_4'){const result=group.result.values as unknown as NspsValues;return <span className="score">{result.overall.score} / {result.overall.maxScore}<br/><small>общий балл</small></span>}
...
function DetailedResult({group}:{group:AnswerGroup}){
  ...
  if(group.code==='test_4'){const result=group.result.values as unknown as NspsValues;return <><b>{result.overall.label}: {result.overall.score} из {result.overall.maxScore}</b><SupportProfile>{Object.values(result.scales).map(scale=><div className="scale" key={scale.label}><span className="name">{scale.label}</span><div className="track"><div className="fill" style={{width:`${(scale.score-scale.minScore)/(scale.maxScore-scale.minScore)*100}%`}}/></div><span className="value">{scale.score}</span><div className="caption">диапазон {scale.minScore}–{scale.maxScore} · чем выше балл, тем сильнее обеспокоенность</div></div>)}</SupportProfile></>}
